...
class RentalRequestService {
  ...
  async create(requestData) {
    try {
      ...
      const docRef = await addDoc(collection(db, RENTAL_REQUESTS_COLLECTION), request);
      return docRef.id;
    } catch (error) {
  ...
  async getById(requestId) {
    try {
      ...
      const requestRef = doc(db, RENTAL_REQUESTS_COLLECTION, requestId);
      ...
      throw new Error(error.message || 'Failed to fetch rental request');
    }
  }
  ...
  async updateStatus(requestId, status, propertyTitle, userId) {
    try {
      ...
      throw new Error(error.message || 'Failed to update rental request');
    }
  }
  ...
  async delete(requestId) {
    try {
      ...
      throw new Error(error.message || 'Failed to delete rental request');
    }
  }
