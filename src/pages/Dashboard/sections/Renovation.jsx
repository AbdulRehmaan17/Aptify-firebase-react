...
import { Wrench, Plus, Eye, Trash2, Edit, DollarSign, Calendar } from 'lucide-react';
...
const Renovation = ({ user, onDataReload }) => {
  ...
  const loadRequests = async () => {
    ...
    setLoading(true);
    try {
      const renovationRequests = await renovationRequestService.getByUser(user.uid);
      setRequests(renovationRequests);
    } catch (error) {
      console.error('Error loading renovation requests:', error);
      toast.error('Failed to load renovation requests');
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await renovationRequestService.delete(requestToDelete.id);
      toast.success('Renovation request deleted');
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-textSecondary mt-2">Manage your renovation service requests</p>
        </div>
      ...
      {requests.length === 0 ? (
        <div className="bg-surface rounded-lg shadow-md p-12 text-center border border-muted">
          ...
          <h3 className="text-lg font-medium text-textMain mb-2">No renovation requests</h3>
          <p className="text-textSecondary mb-4">Start by creating a new renovation request</p>
          <Button onClick={() => navigate('/renovation-request')} variant="primary">
            Create Request
          </Button>
        </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <h3 className="text-lg font-semibold text-textMain">
                      {request.serviceCategory || 'Renovation Request'}
                    </h3>
                ...
                <div className="flex items-center gap-2 ml-4">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => navigate(`/renovation/${request.id}`)}
                  >
                  ...
                  {request.status === 'pending' && (
                    <>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => navigate(`/renovation-request?edit=${request.id}`)}
                      >
      ...
      >
        <div className="space-y-4">
          <p className="text-textMain">
            Are you sure you want to delete this renovation request? This action cannot be undone.
          </p>
